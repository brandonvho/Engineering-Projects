...
- Reducing tube clearance to improve occlusion increases the compression force required from the bearing, which increases the torque demand on the stepper motor. The arm radius directly controls this — a shorter arm requires less torque to produce the same compression force. The current arm radius may be too large for the 28BYJ-48 to handle.
- It is not yet clear what the optimal rotor geometry is — the right arm radius, bearing size, and arm profile need to be worked out before reprinting.

**Next**
- Research the torque budget: calculate how much compression force the motor can actually deliver at the current arm radius, and determine what radius would bring it within the motor's capability.
